...
{
    private static readonly ConcurrentDictionary<string, bool> _validatedThumbnailFormats = new(StringComparer.OrdinalIgnoreCase);
    #region Fields
    ...
    protected virtual async Task<bool> GeneratedThumbExistsAsync(string thumbFilePath, string thumbFileName)
    {
        ...
        {
            _validatedThumbnailFormats.TryRemove(thumbFilePath, out _);
            return false;
        }

        if (_validatedThumbnailFormats.TryGetValue(thumbFilePath, out var valid))
            return valid;

        var binary = await _fileProvider.ReadAllBytesAsync(thumbFilePath);
        valid = IsEncodedFormatCompatible(binary, _fileProvider.GetFileExtension(thumbFileName));
        _validatedThumbnailFormats[thumbFilePath] = valid;
        return valid;
    }
    ...
    protected virtual async Task SaveThumbAsync(string thumbFilePath, string thumbFileName, string mimeType, byte[] binary)
    {
        ...
        // Keep generated bytes consistent with the URL extension/MIME. This
        // repairs legacy caches where a WebP body was stored under .jpeg.
        binary = NormalizeEncodedFormat(binary, mimeType);

        //save
        await _fileProvider.WriteAllBytesAsync(thumbFilePath, binary);
        _validatedThumbnailFormats[thumbFilePath] = true;
    }

    protected virtual bool IsEncodedFormatCompatible(byte[] binary, string extension)
    {
        ...
        {
            "jpg" or "jpeg" => binary[0] == 0xFF && binary[1] == 0xD8 && binary[2] == 0xFF,
            "png" => binary[0] == 0x89 && binary[1] == 0x50 && binary[2] == 0x4E && binary[3] == 0x47,
            "gif" => binary[0] == 0x47 && binary[1] == 0x49 && binary[2] == 0x46,
            "webp" => binary.Length >= 12 && binary[0] == 0x52 && binary[1] == 0x49 && binary[2] == 0x46 && binary[3] == 0x46
                && binary[8] == 0x57 && binary[9] == 0x45 && binary[10] == 0x42 && binary[11] == 0x50,
            _ => true
        };
    }

    protected virtual byte[] NormalizeEncodedFormat(byte[] binary, string mimeType)
    {
        var extension = mimeType?.Split('/').LastOrDefault();
        if (binary == null || binary.Length == 0 || IsEncodedFormatCompatible(binary, extension))
            return binary;

        try
        {
            using var image = SKBitmap.Decode(binary);
            if (image == null)
                return binary;

            using var encodedImage = SKImage.FromBitmap(image);
            return encodedImage.Encode(GetImageFormatByMimeType(mimeType), _mediaSettings.DefaultImageQuality > 0 ? _mediaSettings.DefaultImageQuality : 80).ToArray();
        }
        catch
        {
            return binary;
