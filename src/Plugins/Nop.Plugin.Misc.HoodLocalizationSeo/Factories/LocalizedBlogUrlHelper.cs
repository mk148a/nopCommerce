...
public sealed class LocalizedBlogUrlHelper : INopUrlHelper
{
    ...
    public async Task<string> RouteGenericUrlAsync<TEntity>(TEntity entity, string protocol = null,
        string host = null, string fragment = null, int? languageId = null,
        bool ensureTwoPublishedLanguages = true)
        where TEntity : BaseEntity, ISlugSupported
    {
        if (typeof(TEntity) != typeof(BlogPost) || !_localizationSettings.SeoFriendlyUrlsForLanguagesEnabled)
            return await _inner.RouteGenericUrlAsync(entity, protocol, host, fragment, languageId,
                ensureTwoPublishedLanguages);

        var language = await _routeLanguageResolver.ResolveAsync();
        var url = await _inner.RouteGenericUrlAsync(entity, protocol, host, fragment,
            language?.Id ?? languageId, ensureTwoPublishedLanguages);
        return RewriteLanguagePrefix(url, language?.UniqueSeoCode);
    }
    ...
    public string RouteUrl(string routeName, object values = null, string protocol = null,
        string host = null, string fragment = null)
    {
        return _inner.RouteUrl(routeName, values, protocol, host, fragment);
    }
