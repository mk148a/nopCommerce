...
public interface IProductProductionTimeService
{
    ...
    Task<ProductProductionTimeModel> ExtractFromDescriptionAsync(int productId, bool save = false);
    ...
    Task<(int Scanned, int Saved, int Skipped, int Errors)> BulkExtractFromDescriptionsAsync(bool onlyMissing = true, int maxProducts = 1000, bool publishedOnly = true);
}
