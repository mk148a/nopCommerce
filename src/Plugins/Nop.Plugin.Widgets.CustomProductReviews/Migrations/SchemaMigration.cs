...
namespace Nop.Plugin.Widgets.CustomProductReviews.Migrations
{
    ...
    public class SchemaMigration : FluentMigrator.Migration
    {
        ...
        public override void Up()
        {
            ...
            IfDatabase(databaseType)
                .Execute.Sql("BEGIN TRANSACTION");

            try
            {
                CreateOrAlterTable<ProductReviewVideo>();
                CreateOrAlterTable<CustomProductReviewMapping>();
                CreateOrAlterTable<ProductReviewVideoBinary>();
        ...
        {
            // Silme işlemleri için transaction
            IfDatabase(databaseType)
                .Execute.Sql("BEGIN TRANSACTION");

            try
            {
                DeleteTableIfEmpty<CustomProductReviewMapping>();
                DeleteTableIfEmpty<ProductReviewVideoBinary>();
                DeleteTableIfEmpty<ProductReviewVideo>();
        ...
        private void CreateOrAlterTable<TEntity>() where TEntity : BaseEntity
        {
            ...
            foreach (var expected in expectedColumns)
            {
                ...
                else
                {
                    ...
                    if (needsAlter)
                    {
                        // Mevcut veri kaybını önlemek için sadece NULLABLE değişiklikleri
                        //Alter.Column(expected.ColumnName)
                        //    .OnTable(tableName)
                        //    .AsCustom(expected.SqlDataType)
